...
## `2.` Хранение
### `2.1.` Основной формат
В начал начале указывается *(или не указывается)* 1 байт, который указывает на тип.
Первый байт файла отвечает за указание типа хранения данных.

Если значение первого байта это `01` (1), то используется формат хранения данных в нескольких документах:
```py
File {
    Byte        type      (1);
    Path[]      paths     (N);
